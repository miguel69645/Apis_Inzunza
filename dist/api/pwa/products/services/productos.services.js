...
export const AddOneProdServ = async newProdServ => {
  ...
  try {
    ...
    const ProdServAdded = await CatProdServ.insertMany(newProdServ, {
      order: true
    }).then(prodServ => {
      if (!prodServ) {
        data.status = 400;
...
export const UpdateOneProdServ = async (body, params) => {
  ...
  try {
    ...
    const ProdServUpdated = await CatProdServ.findOneAndUpdate(query, body).then(institute => {
      if (!institute) {
        data.status = 404;
...
export const DeleteOneProdServ = async params => {
  ...
  try {
    ...
    const ProdServDeleted = await CatProdServ.findOneAndDelete(query).then(prodServ => {
      if (!prodServ) {
        data.status = 404;
