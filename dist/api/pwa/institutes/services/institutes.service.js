...
export const addInstitutes = async newInstitute => {
  ...
  try {
    ...
    const InstituteAdded = await institutos.insertMany(newInstitute, {
      order: true
    }).then(institute => {
      if (!institute) {
        data.status = 400;
...
export const putInstitutes = async (idInstitute, newInstitute) => {
  ...
  try {
    ...
    const InstituteUpdated = await institutos.findOneAndUpdate({
      IdInstitutoOK: idInstitute
    }, newInstitute).then(institute => {
      if (!institute) {
        data.status = 400;
...
export const deleteInstitutes = async idInstitute => {
  ...
  try {
    ...
    const InstituteDeleted = await institutos.findOneAndDelete({
      IdInstitutoOK: id
    }).then(institute => {
      if (!institute) {
        data.status = 400;
